...
def maze_traversal(current_room, map = set(), path = []):
    ...
    for next_room in graph[current_room]:
        ...
        if graph[current_room][next_room] == '?':
            ...
            maze_traversal(current_room_id, map, path)
    ...
    while len(queue) > 0:
        ...
        for exits in graph[current_room]:
            ...
            if graph[current_room][exits] == '?':
                ...
                return
            if graph[current_room][exits] not in queue and graph[current_room][exits] not in visited:
                copy = queue[0][:]
                print("copy: ", copy)
                copy.append(graph[current_room][exits])
                queue.append(copy)
